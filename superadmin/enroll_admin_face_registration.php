...
$page_title = 'Admin Registration • Face Enrollment';
...
try {
    ...
    $stmt = $pdo->prepare('SELECT name, email, student_id, created_by_superadmin_id, expires_at FROM admin_face_registration_tokens WHERE token = ? AND expires_at > UTC_TIMESTAMP() LIMIT 1');
    $stmt->execute([$token]);
...
<div class="mb-6 fade-in">
    <div class="glass-effect rounded-xl p-4 sm:p-6 shadow-lg">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Admin Registration</h1>
                <p class="text-slate-600 mt-1">Complete face enrollment to create the admin account.</p>
                <p class="text-slate-600 mt-2">Admin: <span class="font-semibold text-slate-800"><?php echo e((string)$reg['name']); ?></span> • <?php echo e((string)$reg['email']); ?></p>
                <p class="text-slate-500 text-sm mt-1">Admin ID: <?php echo e((string)$reg['student_id']); ?> • Expires: <?php echo e((string)$reg['expires_at']); ?></p>
            </div>
...
<?php if (!$faceEnabled): ?>
    <div class="glass-effect rounded-xl p-6 shadow-lg">
        <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-xl">
            ...
            <p class="text-sm mt-1">Set <span class="font-mono">FACE_RECOGNITION_ENABLED=true</span> to enable admin registration with face enrollment.</p>
        </div>
    </div>
<?php else: ?>
    ...
    <div class="flex flex-col lg:flex-row gap-6">
        <div class="flex-1">
            ...
            <div class="text-center mb-4">
                ...
                <p class="text-slate-500 text-sm mt-1">If the system detects this face is already enrolled as a student, it will block the registration.</p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <button id="btnFinalize" class="flex-1 px-4 py-3 bg-slate-900 text-white rounded-xl font-semibold disabled:opacity-50 disabled:cursor-not-allowed" disabled>
        ...
        <div class="w-full lg:w-80">
            <div class="bg-slate-50 border border-slate-200 rounded-xl p-4">
                ...
                <div class="mt-4">
                    <p class="text-xs font-medium text-slate-500">Captured:</p>
...
const regToken = <?php echo json_encode($token); ?>;
...
function renderCaptured() {
    ...
    const complete = requiredAngles.every(a => set.has(a));
    document.getElementById('btnFinalize').disabled = !complete;
}
...
async function start() {
    ...
    if (currentStep >= requiredAngles.length) {
        updateStepsUI();
        setStatus('✅ Face enrollment complete. Click "Create Admin Account" to finish.', 'success');
    ...
    document.getElementById('btnCapture').disabled = (currentStep >= requiredAngles.length);
...
async function capture() {
    ...
    try {
        const result = await apiPostJson('stage_admin_registration_face.php', {
            token: regToken,
        ...
        if (!result?.success) {
            throw new Error(result?.error || 'Failed to stage face.');
        }
        ...
        if (err?.status === 409 && err?.payload?.conflict) {
            const c = err.payload.conflict;
            setStatus('❌ Enrollment blocked: this face matches an existing student (' + (c.student_name || 'Unknown') + ' - ' + (c.student_id || 'N/A') + ').', 'error');
            Swal.fire({
                icon: 'error',
                title: 'Face Already Enrolled',
                text: 'This face is already enrolled to a student account. Admin registration is blocked.',
                confirmButtonColor: '#0056b3'
            });
        } else {
            setStatus('❌ ' + (err?.message || 'Failed to stage face.'), 'error');
        }
        btn.disabled = false;
    }
}
...
async function finalize() {
    ...
    btn.disabled = true;
    setStatus('✅ Finalizing admin registration…', 'info');

    try {
        const result = await apiPostJson('finalize_admin_registration.php', { token: regToken });

        if (liveTimer) { clearInterval(liveTimer); liveTimer = null; }
        try { if (faceSystem) faceSystem.stopCamera(); } catch (e) {}

        setStatus('🎉 ' + (result?.message || 'Admin created successfully.'), 'success');
        Swal.fire({
            icon: 'success',
            title: 'Admin Created!',
            text: result?.message || 'Admin account has been created successfully.',
            confirmButtonColor: '#0056b3'
        }).then(() => {
        ...
        setStatus('❌ ' + (err?.message || 'Failed to finalize registration.'), 'error');
        btn.disabled = false;
    }
}
...
function cancel() {
    ...
    apiPostJson('cancel_admin_registration.php', { token: regToken })
        .catch(() => {})
        .finally(() => {
            window.location.href = 'homepage.php';
        });
